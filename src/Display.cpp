...
void Display ::swop(uint16_t point1, uint16_t point2) {
    uint16_t temp;
    temp = point1;
    point1 = point2;
    point2 = temp;
...
void Display ::newImage(uint16_t imageName) {
...
void Display ::drawPixel(uint16_t xPoint, uint16_t yPoint, uint16_t color) {
...
void Display ::clearWindow(uint16_t color) {
    uint16_t height = image.memoryHeight;
    uint16_t width = (image.memoryWidth % 8 == 0) ? (image.memoryWidth / 8)
                                                  : (image.memoryWidth / 8 + 1);
    uint32_t offset = image.imageOffset;
...
void Display ::setPixel(uint16_t xPoint, uint16_t yPoint, uint16_t color) {
    uint16_t x, y;
...
void Display ::drawPoint(uint16_t xPoint, uint16_t yPoint) {
    uint16_t color = _color;
...
void Display ::drawLine(uint16_t xStart, uint16_t yStart, uint16_t xEnd,
                        uint16_t yEnd) {
    uint16_t color = _color;
...
void Display ::drawRectangle(uint16_t xStart, uint16_t yStart, uint16_t xEnd,
                             uint16_t yEnd) {
    uint16_t color = _color;
...
void Display ::drawCircle(uint16_t xCenter, uint16_t yCenter, uint16_t radius) {
    uint16_t color = _color;
    draw_fill draw_fill = _draw_fill;
    dot_pixel dot_pixel = _dot_pixel;
...
void Display ::drawChar(uint16_t xPoint, uint16_t yPoint, const char asciiChar,
...
void Display ::print(uint16_t xStart, uint16_t yStart, const char *pString) {
...
void Display ::print(uint16_t xStart, uint16_t yStart, int32_t num) {
    String str = String(num);
    print(xStart, yStart, str.c_str());
}

void Display ::print(uint16_t xStart, uint16_t yStart, String s) {
    print(xStart, yStart, s.c_str());
}

void Display ::drawBitMap(const unsigned char *imageBuffer) {
...
void Display ::sendCommand(uint8_t reg) {
    digitalWrite(EPD_DC, 0);
    digitalWrite(EPD_CS, 0);
    SPI.transfer(reg);
    digitalWrite(EPD_CS, 1);
}
...
void Display::readBusy(void) {
    while (digitalRead(EPD_BUSY) == 0) {  //LOW: idle, HIGH: busy
        delay(100);
    }
}