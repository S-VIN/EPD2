...
void B7in5::turnOnDisplay(void) {
    sendCommand(0x12);  // DISPLAY_REFRESH
    delay(100);
    readBusy();
}

void B7in5::init(void) {
    reset();
    ...
    sendData(_width >> 8);  // source 640
    sendData(_width & 0xff);
    sendData(_height >> 8);  // gate 384
...
void B7in5::setColor(uint8_t color) {
    if (color == black) {
        _color = white;
    } else {
        _color = black;
...
void B7in5 ::clearWindow(uint16_t color) {
    if (color == black) {
        color = white;
    } else {
        color = black;
    ...
    uint16_t width = (image.memoryWidth % 8 == 0) ? (image.memoryWidth / 8) : (image.memoryWidth / 8 + 1);
