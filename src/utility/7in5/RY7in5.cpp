...
void RY7in5 ::turnOnDisplay(void) {
    sendCommand(0x04);  
    readBusy();
    sendCommand(0x12); 
    delay(100);
...
void RY7in5 ::displayHalfScreen(void) {
    uint8_t Data_Black, Data_RY, data;
    uint32_t i, j, width, height;
    width = (_width % 8 == 0) ? (_width / 8) : (_width / 8 + 1);
    height = _height;
